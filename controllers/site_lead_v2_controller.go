...
func CreateSiteLeadV2(c *fiber.Ctx) error {
	...
	payment, pix, err := pkg.ChargeWebsiteCheckout(
		siteLeadV2ToSolicitation(lead),
		lead.Amount,
		buildSiteLeadV2Description(lead),
	)
	if err != nil {
	...
	if err := applyPaymentToSiteLeadV2(&lead, payment, &pix); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"ok":      false,
	...
	return c.Status(fiber.StatusCreated).JSON(siteLeadV2Response(lead, "Lead registrado no sistema", ""))
}

func GetSiteLeadStatusV2(c *fiber.Ctx) error {
	...
	warning := ""
	if lead.AsaasPaymentID != "" && !isPaidStatus(lead.PaymentStatus) && !strings.HasPrefix(lead.AsaasPaymentID, "pending:v2:") {
		payment, err := pkg.GetPayment(lead.AsaasPaymentID)
		if err != nil {
			warning = err.Error()
		} else if err := applyPaymentToSiteLeadV2(&lead, payment, nil); err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"ok":      false,
	...
	return c.JSON(siteLeadV2Response(lead, "Lead registrado no sistema", warning))
}

func HandleAsaasWebhookV2(c *fiber.Ctx) error {
	payload, err := parseAsaasWebhookPayload(c)
	if err != nil {
		fmt.Println("HandleAsaasWebhookV2: cannot parse webhook, acking to avoid Asaas queue penalty", err.Error())
	...
	if paymentID == "" {
		fmt.Println("HandleAsaasWebhookV2: missing payment id, body=", string(c.Body()))
		return c.JSON(fiber.Map{
			"ok":      true,
			"ignored": true,
	...
	if err := finalizeReleasedLeadV2(&lead); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"ok":      false,
...
func buildSiteLeadV2Description(lead models.SiteLeadV2) string {
	...
	return fmt.Sprintf("Solicitacao via site v2 | %s", modeLabel)
}

func applyPaymentToSiteLeadV2(lead *models.SiteLeadV2, payment models.ASAASPayment, pix *models.ASAASPixResponse) error {
...
func siteLeadV2Response(lead models.SiteLeadV2, message, warning string) fiber.Map {
	response := fiber.Map{
		"ok":      true,
		"message": message,
		"leadId":  lead.ID,
...
func buildSiteLeadV2Mail(lead models.SiteLeadV2) string {
		...
		<div style="font-family:Arial,sans-serif;background:#f5f7fb;padding:24px;">
		  <div style="max-width:760px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7f0;">
			<div style="background:#101322;color:#ffffff;padding:20px 24px;">
			  ...
			  <h1 style="margin:8px 0 0;font-size:24px;line-height:1.2;">Lead v2 liberado apos pagamento PIX</h1>

...
type SiteLead struct {
	...
	AsaasPaymentID    string     `json:"asaas_payment_id" gorm:"size:80;index"`
...
func (s *SiteLead) BeforeCreate(tx *gorm.DB) (err error) {
	if s.ID == uuid.Nil {
		s.ID = uuid.New()
	}
	if s.AsaasPaymentID == "" {
		s.AsaasPaymentID = fmt.Sprintf("pending:%s", s.ID.String())
	}
	return
}
